...
const Form = () => {
	...
	const handleNumMiniQuestionsChange = (subquestionIndex, typeIndex, e) => {
		...
		if (numMiniQuestions < currentCount) {
			...
			const hasContent = questionsToRemove.some(
				(q) =>
			...
			if (hasContent) {
				alert(
					`Cannot decrease to ${numMiniQuestions} question(s). Please clear the content of question ${numMiniQuestions + 1} onwards before reducing the count.`,
				);
		...
		if (numMiniQuestions > 0) {
			...
			if (numMiniQuestions > currentCount) {
				...
				types[typeIndex].miniQuestions = [
					...currentMiniQuestions,
					...newQuestions,
				];
			} else if (numMiniQuestions < currentCount) {
				types[typeIndex].miniQuestions = currentMiniQuestions.slice(
					0,
					numMiniQuestions,
				);
			}
				...
				if (
					...
					const mappedSubQuestions = questionData.subQuestions.map(
						(subQuestion) => {
							...
							const types = subQuestionsData.map((type) => {
								...
								console.log(
									`subtype: ${type.subtype}, _id: ${type._id}`,
								);

								const subtypeValue = type.subtype;
								const isCorrupted = /^[0-9a-f]{24}$/i.test(
									subtypeValue,
								);
								if (isCorrupted) {
									console.error(
										`CORRUPTED DATA: subtype is an ObjectId: ${subtypeValue}`,
									);
									console.error(
										'You need to delete this document from MongoDB and recreate it!',
									);
								}
								...
								return {
									label: getSubtypeLabel(type.subtype),
									id: type.subtype,
					...
					setSubquestions((prevState) => {
						const fetchedMap = new Map(
							mappedSubQuestions
								.filter(
									(sq) => sq.types && sq.types.length > 0,
								)
								.map((sq) => [sq.label.toLowerCase(), sq]),
						);

						return prevState.map((phase) => {
							const fetched = fetchedMap.get(
								phase.label.toLowerCase(),
							);
							if (fetched) {
								console.log(
									`Updating ${phase.label} with fetched data (${fetched.types.length} types)`,
								);
								return fetched;
							}
							console.log(
								`Keeping initial state for ${phase.label} (not in DB or empty)`,
							);
							return phase;
						});
					});
	...
	const handleSave = (questionIndex) => {
		...
		if (!isSubquestionEmpty) {
			if (isCalculationType && !hasMiniQuestions) {
				...
				const validTypes = subquestion.types.filter(
					(type) =>
						type.miniQuestions && type.miniQuestions.length > 0,
				);

				console.log(
					'Total types in subquestion:',
					subquestion.types.length,
				);
				...
				const questions = validTypes.map((type) => {
					...
					if (isObjectId) {
						console.error(
							`CRITICAL ERROR: type.id is an ObjectId! ${subtype}`,
						);
						console.error(`Type label: ${type.label}`);
						console.error(
							'This should never happen! Using label as fallback.',
						);
					...
					console.log(
						`Processing type: ${type.label} (id: ${type.id}, subtype: ${subtype}, isObjectId: ${isObjectId})`,
					);
					console.log(
						`Mini questions count: ${type.miniQuestions.length}`,
					);
				...
				const uniqueQuestions = questions.filter(
					(q, index, self) =>
						index ===
						self.findIndex((t) => t.subtype === q.subtype),
				);

				if (uniqueQuestions.length !== questions.length) {
					console.warn(
						`Removed ${questions.length - uniqueQuestions.length} duplicate subtypes`,
					);
				}
		...
		<div className="make-questions">
			<div className="form-container">
				<div className="form-hero">
					<p className="form-kicker">Teacher Workspace</p>
					<h2 className="form-heading">Question Builder</h2>
					<p className="form-hero-copy">
						Create or update an estimation problem, upload its core media,
						and define the prompts students will see at each phase.
					</p>
				</div>
				<form className="form">
					<div className="editor-card editor-card--main">
						<div className="editor-card__header">
							<p className="editor-card__eyebrow">Problem Setup</p>
							<h3 className="editor-card__title">
								Main question and learning assets
							</h3>
						</div>
						<div className="form-group">
							...
							<input
								...
								placeholder="Enter the main estimation problem statement"
							/>
						</div>
						...
						{selectedImageURL && (
							<div className="selected-image-container">
								<span className="form-label">Selected Image Preview</span>
								<img
						...
						<div className="form-group form-group--actions">
							<button
								type="button"
								className="save-button save-button--accent"
								onClick={handleSaveInfoCenter}
							>
								{questionId ? 'Update' : 'Save'} main question and assets
							</button>
						</div>
					</div>
					{subquestions.map((subquestion, subquestionIndex) => (
						<div className="subquestion editor-card" key={subquestionIndex}>
							<div className="editor-card__header">
								<p className="editor-card__eyebrow">Subquestion</p>
								<h3 className="editor-card__title">
									{subquestion.label} prompts
								</h3>
							</div>
							<div className="form-group">
										...
										<div className="mini-questions-container">
											{type.miniQuestions.map(
												(
													...
													<div
														className="form-group mini-question mini-question-card"
														key={miniQuestionIndex}
													>
														<label
															className="form-label mini-question-card__label"
														>
															{type.label} - Question{' '}
															{miniQuestionIndex + 1}
															:
