...
export default function FunctionalPlanScreen() {
		...
		const getData = async () => {
			try {
				...
				const res2 = await fetch(
					`${apiurl}/answer/type?sessionId=${sessionId}&type=functional&subtype=plan`,
					{
						headers: {
							'Content-Type': 'application/json',
							Authorization: `Bearer ${localStorage.getItem(
	...
	const handleChange = (event) => {
		...
		sessionSocket.emit('forward', {
			sessionId: sessionId,
			eventDesc: `funcplan-answer-typing`,
			value: {
				...answerData,
				[id]: data,
			},
	...
	sessionSocket.on('forward', (data) => {
		if (data.eventDesc === 'funcplan-answer-typing') {
			setAnswerData(data.value);
		}

		if (data.eventDesc === 'funcplan--navigate') {
			navigate(data.path);
		}
	});
